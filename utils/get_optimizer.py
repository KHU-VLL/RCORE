...
def get_optimizer_vlm(cfg,model):
    ...
    if cfg.text_encoding_manner=='composition':
        ...
        if not isinstance(model.verb_prompt_learner, torch.Tensor):
            for name, param in model.verb_prompt_learner.named_parameters():
                prompt_param.append(param)
        if hasattr(model, 'obj_prompt_learner'):
            if not isinstance(model.obj_prompt_learner, torch.Tensor):
        ...
        for name, param in model.named_parameters():
            if 'c2c' in name:
                c2c_with_wd.append(param)
            if 'agg' in name and 'bias' not in name and 'norm' not in name :
                agg_param_with_wd.append(param)
            elif 'agg' in name :
                agg_param_no_wd.append(param)
        ...
        optimizer = torch.optim.AdamW([
            ...
            {'params': agg_param_with_wd, 'lr': cfg.visual_lr * 0.1, 'weight_decay': cfg.visual_wd},
            {'params': agg_param_no_wd, 'lr': cfg.visual_lr * 0.1, 'weight_decay': 0.0},],

            betas=(0.9, 0.999), lr=cfg.visual_lr, eps=1e-8,
            weight_decay=cfg.visual_wd)  # Params used from paper, the lr is
    elif cfg.text_encoding_manner == 'siglip_component':
        for name, param in model.named_parameters():
            if 'c2c' in name:
                c2c_with_wd.append(param)
        optimizer = torch.optim.AdamW([
