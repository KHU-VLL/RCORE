...
#sth-else: NOTE: Returns [0-255] rather than torchvision's [0-1]
#now: return torchvision's [0-1]

class ToTensor(object):
    def __init__(self):
        # self.worker = lambda x: F.to_tensor(x) * 255
        self.worker = lambda x: F.to_tensor(x)
...
class ToPIL(object):
    def __init__(self):
        self.worker = ToPILImage()

    def __call__(self, img_group):
        img_group = [self.worker(img) if isinstance(img, torch.Tensor) else img for img in img_group]
        return img_group
