...
class PretrainInternVideo2(nn.Module):
    ...
    # @torch.cuda.amp.autocast(enabled=False)
    def forward(self, x, mask=None, use_image=False, x_vis_return_idx=-1, return_all_features=False):
        ...
        if return_all_features :
            ...
            patch_tokens = x[:, 1:, :]      # (B, 2048, D)
            
            # 2. Frame token Reshape: (B, 8, 256, D)
            patch_tokens = patch_tokens.view(B, T, L, C)
            
            # 3. CLS token to each frame: (B, 8, 1, D)
            cls_expanded = cls_token.unsqueeze(1).expand(-1, T, -1, -1)
            
            # 4. Concat (256 + 1 = 257): (B, 8, 257, D)
            combined = torch.cat([cls_expanded, patch_tokens], dim=2)
            
            # 5. Mean: (B, 8, 768)
            new_x = combined.view(B*T, -1, C)
            new_x = self.clip_projector(new_x)  # attn pool (B*T, 257, D) -> (B*T, 1, D)
            return new_x.view(B, T, -1)
...
def pretrain_internvideo2_1b_patch14_224(config):
    model = PretrainInternVideo2(
        ...
        use_flash_attn=getattr(config.vision_encoder, 'use_flash_attn', True),
        use_fused_rmsnorm=getattr(config.vision_encoder, 'use_fused_rmsnorm', True),
        use_fused_mlp=getattr(config.vision_encoder, 'use_fused_mlp', True),
...
def pretrain_internvideo2_6b_patch14_224(config):
    ...
    if config.vision_encoder.pretrained is not None:
        logger.info(f"Loading pretrained weights from {config.vision_encoder.pretrained}")
        state_dict = torch.load(config.vision_encoder.pretrained, map_location='cpu')
        interpolate_pos_embed_internvideo2(state_dict, model, orig_t_size=config.vision_encoder.num_frames)
