...
class TextTransformer(nn.Module):
    ...
    def encode_text(
        ...
        input_prompts=None,  # custom for CoOp
        *args,
        **kwargs
    ) -> Tensor:
        ...
        # [batch_size, context_length] --> [batch_size, context_length, hidden_dim]
        if input_prompts is not None :  # custom for CoOp
        ...
        if input_prompts is not None :  # custom for CoOp
            token_emb = token_emb[
                torch.arange(input_prompts.shape[0]), input_prompts.argmax(dim=1)
            ]
        else :
            token_emb = token_emb[
    ...
    def forward(
        ...
        key_padding_mask: Optional[Tensor] = None,
        return_all_tokens: bool = False,
        input_prompts = False,   # custom for CoOp
        *args,
        **kwargs
    ) -> Tensor:
        ...
        text_tokens = self.encode_text(
            ...
            input_prompts=input_prompts,  # custom for CoOp
            *args,
            **kwargs
        )
        return text_tokens
